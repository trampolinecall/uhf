...
type IRReader = Reader SIR.SIR

type DumpableConstraints = (Show Int)
    -- ( DumpableIdentifier (SIR.DIdenStart)
    -- , DumpableIdentifier (SIR.VIdenStart)
    -- , DumpableIdentifier (SIR.PIdenStart)
    -- )

-- TODO: be able to print resolved identifiers here (just rewrite this entire module?)

dump_main_module :: DumpableConstraints => SIR.SIR -> Text
...
get_adt_arena :: IRReader (Arena.Arena (Type.ADT (SIR.TypeExpr, SIR.ID.ID "TypeExprEvaledAsType")) Type.ADTKey)
get_adt_arena = reader (\ (SIR.SIR _ adts _ _ _ _) -> adts)
get_type_synonym_arena :: IRReader (Arena.Arena (Type.TypeSynonym (SIR.TypeExpr, SIR.ID.ID "TypeExprEvaledAsType")) Type.TypeSynonymKey)
...
get_var :: SIR.VariableKey -> IRReader (SIR.Variable)
get_var k = reader (\ (SIR.SIR _ _ _ _ vars _) -> Arena.get vars k)
get_module :: SIR.ModuleKey -> IRReader (SIR.Module)
get_module k = reader (\ (SIR.SIR modules _ _ _ _ _) -> Arena.get modules k)
get_adt :: Type.ADTKey -> IRReader (Type.ADT (SIR.TypeExpr, SIR.ID.ID "TypeExprEvaledAsType"))
get_adt k = reader (\ (SIR.SIR _ adts _ _ _ _) -> Arena.get adts k)
get_type_syn :: Type.TypeSynonymKey -> IRReader (Type.TypeSynonym (SIR.TypeExpr, SIR.ID.ID "TypeExprEvaledAsType"))
...
define_module :: DumpableConstraints => SIR.Module -> IRReader PP.Token
...
define_binding :: DumpableConstraints => SIR.Binding -> IRReader PP.Token
define_binding (SIR.Binding _ pat _ init) = pattern pat >>= \ pat -> expr init >>= \ init -> pure $ PP.List [pat, " = ", init, ";"]

refer_var :: SIR.VariableKey -> IRReader PP.Token
...
-- refer_bv :: SIR.ValueRef -> IRReader PP.Token
...
-- TODO: rename this to something better
-- -- TODO: remove this and print it through annotations
-- refer_decl :: DumpableType t => SIR.DeclRef t -> IRReader PP.Token
...
-- TODO: remove this and print it through annotations
-- refer_adt_variant :: Type.ADT.VariantIndex -> IRReader PP.Token
...
class DumpableType ty where
    refer_type :: ty -> IRReader PP.Token

instance DumpableType TypeWithInferVar.Type where
    refer_type t = do
...
class DumpableIdentifier i where
    refer_iden :: i -> IRReader PP.Token

instance DumpableIdentifier a => DumpableIdentifier (Located a) where
    refer_iden = refer_iden . unlocate
instance DumpableIdentifier t => DumpableIdentifier (Maybe t) where -- TODO: remove this
    refer_iden (Just t) = refer_iden t
    refer_iden Nothing = pure $ PP.String "<name resolution error>"

instance DumpableIdentifier Text where
    refer_iden = pure . PP.String

-- TODO: remove these
-- instance DumpableType t => DumpableIdentifier (SIR.DeclRef t) where
--     refer_iden = refer_decl
-- instance DumpableIdentifier SIR.ValueRef where
--     refer_iden = refer_bv
-- instance DumpableIdentifier Type.ADT.VariantIndex where
--     refer_iden = refer_adt_variant

-- TODO: figure out how to overload this for if resolved is not ()
refer_split_iden :: (DumpableConstraints) => SIR.SplitIdentifier id_name -> IRReader PP.Token
...
-- TODO: figure out how to overload this for if resolved is not ()
refer_split_iden_and_resolved :: (DumpableConstraints) => SIR.SplitIdentifier id_name -> resolved -> IRReader PP.Token
...
type_expr :: DumpableConstraints => SIR.TypeExpr -> IRReader PP.Token
...
expr :: DumpableConstraints => SIR.Expr -> IRReader PP.Token
...
pp_let :: DumpableConstraints => Text -> [SIR.Binding] -> [Type.ADTKey] -> [Type.TypeSynonymKey] -> SIR.Expr -> IRReader PP.Token
...
pattern :: DumpableConstraints => SIR.Pattern -> IRReader PP.Token
