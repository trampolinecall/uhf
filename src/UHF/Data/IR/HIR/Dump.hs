...
expr (HIR.Expr'Identifier _ _ _ i) = refer_iden i
expr (HIR.Expr'Char _ _ _ c) = text $ show c
expr (HIR.Expr'String _ _ _ s) = text $ show s
expr (HIR.Expr'Int _ _ _ i) = text $ show i
expr (HIR.Expr'Float _ _ _ (n :% d)) = text $ "(" <> show n <> "/" <> show d <> ")"
expr (HIR.Expr'Bool _ _ _ b) = text $ if b then "true" else "false"
expr (HIR.Expr'Tuple _ _ _ a b) = text "(" >> expr a >> text ", " >> expr b >> text ")"
expr (HIR.Expr'Lambda _ _ _ param body) = text "\\ " >> pattern param >> text " -> " >> expr body -- TODO: decide if this should be \ (x) -> or \ x ->
expr (HIR.Expr'Let _ _ _ bindings body) = text "let {\n" >> lift DumpUtils.indent >> mapM_ define_binding bindings >> lift DumpUtils.dedent >> text "}\n" >> expr body
expr (HIR.Expr'BinaryOps _ _ _ _ first _) = text "(" >> expr first >> todo >> text ")" -- TODO
expr (HIR.Expr'Call _ _ _ callee arg) = text "(" >> expr callee >> text "(" >> expr arg >> text "))"
expr (HIR.Expr'If _ _ _ _ cond t f) = text "if " >> expr cond >> text " then " >> expr t >> text " else " >> expr f
expr (HIR.Expr'Case _ _ _ _ _ _) = todo
expr (HIR.Expr'TypeAnnotation _ _ _ ty e) = text ":" >> refer_type ty >> text ": " >> expr e
