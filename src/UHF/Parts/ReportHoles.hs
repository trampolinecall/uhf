...
report_holes :: (SIR.TypeInfo stage ~ Maybe Type.Type) => TypeExprsFinalEvaledAsTypes -> SIR.SIR stage -> Compiler.WithDiagnostics (Error stage) Void ()
...
module_ :: (SIR.TypeInfo stage ~ Maybe Type.Type) => SIR.ModuleKey -> ReaderT (SIR.SIR stage, TypeExprsFinalEvaledAsTypes) (Compiler.WithDiagnostics (Error stage) Void) ()
module_ key =
...
adt :: (SIR.TypeInfo stage ~ Maybe Type.Type) => Type.ADTKey -> ReaderT (SIR.SIR stage, TypeExprsFinalEvaledAsTypes) (Compiler.WithDiagnostics (Error stage) Void) ()
...
type_synonym :: (SIR.TypeInfo stage ~ Maybe Type.Type) => Type.TypeSynonymKey -> ReaderT (SIR.SIR stage, TypeExprsFinalEvaledAsTypes) (Compiler.WithDiagnostics (Error stage) Void) ()
...
binding :: (SIR.TypeInfo stage ~ Maybe Type.Type) => SIR.Binding stage -> ReaderT (SIR.SIR stage, TypeExprsFinalEvaledAsTypes) (Compiler.WithDiagnostics (Error stage) Void) ()
binding (SIR.Binding _ p _ e) = pattern p >> expr e

pattern :: SIR.Pattern stage -> ReaderT (SIR.SIR stage, TypeExprsFinalEvaledAsTypes) (Compiler.WithDiagnostics (Error stage) Void) ()
...
type_expr (SIR.TypeExpr'Hole _ hid sp hiden) = do
    (_, type_exprs_evaled_as_types) <- ask
    case type_exprs_evaled_as_types Map.! hid of
        Just type_info ->
            ask >>= \ (SIR.SIR _ adts type_synonyms vars _ _, _) ->
            lift (Compiler.tell_error (Error adts type_synonyms vars sp hiden type_info)) >>
            pure ()
        Nothing -> pure () -- typing phase will have already reported ambiguous type
