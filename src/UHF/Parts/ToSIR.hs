...
search_for_main_function mods variables mod =
    ...
    where
        ...
        go_pat (SIR.Pattern'AnonADTVariant _ _ _ _ _ _ field_pats) = concatMap go_pat field_pats
        go_pat (SIR.Pattern'NamedADTVariant _ _ _ _ _ _ field_pats) = concatMap (go_pat . snd) field_pats
...
convert_expr cur_id (AST.Expr'ReferAlpha sp t (Located iden_sp iden)) = SIR.Expr'Refer cur_id () sp <$> (maybe (pure Nothing) (fmap Just . convert_type) t) <*> (pure $ Located iden_sp (convert_aiden_tok iden)) <*> pure ()
            ...
            (\ ind (op, right) -> do
                ...
                case op of
                    AST.Operator'Path op_sp op_ty op_last -> do
                        ...
                        pure (op_sp, Just op_ty, convert_siden_tok <$> op_last, (), right')
                    AST.Operator'Single op_iden@(Located op_sp _) -> pure (op_sp, Nothing, convert_siden_tok <$> op_iden, (), right'))
            [1..]
            ops
...
convert_pattern parent (AST.Pattern'AnonADTVariant sp v_ty variant fields) = do
    ...
    v_ty <- maybe (pure Nothing) (fmap Just . convert_type) v_ty
    pure (SIR.Pattern'AnonADTVariant () sp v_ty (convert_aiden_tok <$> variant) () [] fields)
convert_pattern parent (AST.Pattern'NamedADTVariant sp v_ty variant fields) = do
    ...
    v_ty <- maybe (pure Nothing) (fmap Just . convert_type) v_ty
    pure (SIR.Pattern'NamedADTVariant () sp v_ty (convert_aiden_tok <$> variant) () [] fields)
