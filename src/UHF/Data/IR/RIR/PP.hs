...
define_decl (RIR.Decl'Module bindings adts type_synonyms) =
    ...
    mapM (\ k -> get_type_synonym k >>= pure . PP.OldPP . Type.PP.define_type_synonym (\ ty -> runReader (PP.render <$> refer_m_type ty >>= pure . OldPP.write) rir)) type_synonyms >>= \ type_synonyms ->
...
expr (RIR.Expr'Char _ _ _ c) = pure $ PP.String $ show c
expr (RIR.Expr'String _ _ _ s) = pure $ PP.String $ show s
expr (RIR.Expr'Int _ _ _ i) = pure $ PP.String $ show i
expr (RIR.Expr'Float _ _ _ (n :% d)) = pure $ PP.String $ "(" <> show n <> "/" <> show d <> ")"
...
expr (RIR.Expr'Lambda _ _ _ _ _ param body) = refer_bv param >>= \ param -> expr body >>= \ body -> pure (PP.List ["\\ ", param, " -> ", body]) -- TODO: show captures
expr (RIR.Expr'Let _ _ _ [binding] res) = define_binding binding >>= \ binding -> expr res >>= \ res -> pure (PP.List ["let ", binding, "\n", res])
expr (RIR.Expr'Let _ _ _ bindings res) = expr res >>= \ res -> mapM define_binding bindings >>= \ bindings -> pure (PP.List ["let ", PP.braced_block bindings, "\n", res])
...
expr (RIR.Expr'Seq _ _ _ a b) = expr a >>= \ a -> expr b >>= \ b -> pure $ PP.List ["seq ", a, ", ", b]
...
expr (RIR.Expr'MakeADT _ _ _ variant_index@(Type.ADTVariantIndex adt_key _) args) =
    ...
    in pure $ PP.List ["adt ", PP.OldPP adt_refer, " ", PP.String variant_name, PP.bracketed_comma_list args]
