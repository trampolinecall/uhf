...
lex_delimiter =
    choice
        [ consume (==':') >>= \ c1 -> consume (==':') >>= \ c2 -> pure [Right $ Located (Located.just_span c1 <> Located.just_span c2) (Token.SingleTypeToken Token.DoubleColon)]
        , consume (=='(') >>= \ ch -> pure [Right $ Token.SingleTypeToken Token.OParen <$ ch]
        , consume (==')') >>= \ ch -> pure [Right $ Token.SingleTypeToken Token.CParen <$ ch]
        , consume (=='[') >>= \ ch -> pure [Right $ Token.SingleTypeToken Token.OBrack <$ ch]
        , consume (==']') >>= \ ch -> pure [Right $ Token.SingleTypeToken Token.CBrack <$ ch]
        , consume (=='{') >>= \ ch -> pure [Right $ Token.SingleTypeToken Token.OBrace <$ ch]
        , consume (=='}') >>= \ ch -> pure [Right $ Token.SingleTypeToken Token.CBrace <$ ch]
        ...
        [ consume (==open) >>
          ...
          in if open == '\''
              then case contents of
                    [c] -> pure [Right $ Located sp $ Token.Char c]
                    _ -> pure [Left $ LexError.MulticharCharLit sp]
              else pure [Right $ Located sp $ Token.String $ Text.pack contents]
    ...
    in case (ei_tok_base, floats) of
        (Right (tok_base, base_num), []) ->
            ...
            in if null illegal_digits
                then pure [Right $ Located num_span (Token.Int tok_base (read_digits ((^) :: Integer -> Int -> Integer) base_num (zip [0..] (map Located.unlocate (reverse digits_no_underscore)))))]
        ...
        (Right (tok_base, _), _) ->
            ...
            in if null illegal_digits && null base_is_dec
                then pure [Right $ Located num_span (Token.Float $ read_digits ((^^) :: Rational -> Int -> Rational) 10 (zip [0..] (map Located.unlocate $ reverse digits_no_underscore) ++ zip [-1, -2..] (map Located.unlocate floats)))]
...
case_lex_symbol_identifier_kw =
    minilex_test' lex_symbol_identifier ":" $ \case
        Just (l, [Right (Located _ (Token.SingleTypeToken Token.Colon))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_symbol_identifier" x
case_lex_symbol_identifier_long_kw :: Assertion
case_lex_symbol_identifier_long_kw =
    minilex_test' lex_symbol_identifier "->" $ \case
        Just (l, [Right (Located _ (Token.SingleTypeToken Token.Arrow))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_symbol_identifier" x
...
case_lex_delimiter =
    minilex_test' lex_delimiter "((:))" $ \case
        Just (l, [Right (Located _ (Token.SingleTypeToken Token.OParen))])
            | remaining l == "(:))" -> pure ()
...
case_lex_delimiter_double_colon_then_symbol_identifier =
    minilex_test' lex_delimiter "::*&" $ \case
        Just (l, [Right (Located _ (Token.SingleTypeToken Token.DoubleColon))])
            | remaining l == "*&" -> pure ()
        x -> minilex_test_fail "lex_delimiter" x
...
case_lex_char_lit =
    minilex_test' lex_str_or_char_lit "'c'" $ \case
        Just (l, [Right (Located _ (Token.Char 'c'))])
            | remaining l == "" -> pure ()
...
case_lex_str_lit =
    minilex_test' lex_str_or_char_lit "\"abcde\"" $ \case
        Just (l, [Right (Located _ (Token.String "abcde"))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_str_or_char_lit" x
...
case_lex_number_decimal =
    minilex_test' lex_number "1234" $ \case
        Just (l, [Right (Located _ (Token.Int Token.Dec 1234))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_number" x
...
case_lex_number_float =
    minilex_test' lex_number "1234.1234" $ \case
        Just (l, [Right (Located _ (Token.Float 1234.1234))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_number" x
...
case_lex_number_binary =
    minilex_test' lex_number "0b101" $ \case
        Just (l, [Right (Located _ (Token.Int Token.Bin 5))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_number" x
...
case_lex_number_hex =
    minilex_test' lex_number "0xf1abcABC" $ \case
        Just (l, [Right (Located _ (Token.Int Token.Hex 4054567612))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_number" x
...
case_lex_number_octal =
    minilex_test' lex_number "0o765" $ \case
        Just (l, [Right (Located _ (Token.Int Token.Oct 501))])
            | remaining l == "" -> pure ()
        x -> minilex_test_fail "lex_number" x
...
case_lex_number_with_invalid_float =
    minilex_test' lex_number "123.x" $ \case
        Just (l, [Right (Located _ (Token.Int Token.Dec 123))])
            | remaining l == ".x" -> pure ()
        x -> minilex_test_fail "lex_number" x
