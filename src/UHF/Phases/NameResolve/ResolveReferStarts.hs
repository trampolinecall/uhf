...
resolve sir_child_maps (SIR.SIR decls mods adts type_synonyms type_vars bound_values mod) =
    runStateT
        (
...
resolve_in_mods :: UnresolvedModuleArena -> (Utils.NRReader UnresolvedADTArena UnresolvedBoundValueArena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) (ResolvedModuleArena, Map.Map Type.ADTKey Utils.NameMapStack, Map.Map Type.TypeSynonymKey Utils.NameMapStack)
...
resolve_in_adts :: Map.Map Type.ADTKey Utils.NameMapStack -> UnresolvedADTArena -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) ResolvedADTArena
resolve_in_adts adt_parent_name_maps = Arena.transform_with_keyM (resolve_in_adt adt_parent_name_maps)

resolve_in_type_synonyms :: Map.Map Type.TypeSynonymKey Utils.NameMapStack -> UnresolvedTypeSynonymArena -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) ResolvedTypeSynonymArena
...
resolve_in_module :: SIR.ModuleKey -> SIR.Module Unresolved -> WriterT (Map Type.ADTKey Utils.NameMapStack) (WriterT (Map Type.TypeSynonymKey Utils.NameMapStack) (Utils.NRReader UnresolvedADTArena UnresolvedBoundValueArena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors))) (SIR.Module Resolved)
...
resolve_in_adt :: Map.Map Type.ADTKey Utils.NameMapStack -> Type.ADTKey -> UnresolvedADT -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) ResolvedADT
resolve_in_adt adt_parent_name_maps adt_key (Type.ADT id name type_vars variants) =
    ...
    in mapM
        ...
        type_vars >>= \ type_vars' ->
    lift (lift $ Utils.make_name_maps type_vars' [] []) >>= \ new_nc ->
...
resolve_in_type_synonym :: Map.Map Type.TypeSynonymKey Utils.NameMapStack -> Type.TypeSynonymKey -> UnresolvedTypeSynonym -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) ResolvedTypeSynonym
...
resolve_in_binding :: Utils.NameMapStack -> SIR.Binding Unresolved -> (Utils.NRReader UnresolvedADTArena UnresolvedBoundValueArena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) (SIR.Binding Resolved)
...
resolve_in_type_expr :: Utils.NameMapStack -> SIR.TypeExpr Unresolved -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) (SIR.TypeExpr Resolved)
...
resolve_in_type_expr nc_stack (SIR.TypeExpr'Forall resolved sp vars ty) =
    mapM
        (\ var ->
            ...
            pure (name, Utils.DeclAt name_sp, var_decl))
        (toList vars) >>= \ vars' ->
    lift (lift $ Utils.make_name_maps vars' [] []) >>= \ new_nc ->
...
resolve_in_pat :: Utils.NameMapStack -> SIR.Pattern Unresolved -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) (SIR.Pattern Resolved)
...
resolve_in_expr :: Utils.NameMapStack -> SIR.Expr Unresolved -> (Utils.NRReader UnresolvedADTArena UnresolvedBoundValueArena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) (SIR.Expr Resolved)
...
resolve_in_expr nc_stack (SIR.Expr'Lambda id type_info sp param body) =
    ...
    lift (lift $ Utils.make_name_maps [] param_bvs []) >>= \ new_nc ->
    ...
    SIR.Expr'Match id type_info sp match_tok_sp
        ...
        <*> mapM
                (\ (pat, expr) ->
                    ...
                    lift (lift $ Utils.make_name_maps [] bv_children []) >>= \ arm_nc ->
...
resolve_split_iden :: (Utils.NameMapStack -> UnresolvedIdenStart -> Utils.WithErrors resolved_iden) -> Utils.NameMapStack -> SIR.SplitIdentifier Unresolved UnresolvedIdenStart -> (Utils.NRReader adt_arena bv_arena TypeVarArena Utils.SIRChildMaps (Utils.MakeDeclState Utils.WithErrors)) (SIR.SplitIdentifier Resolved resolved_iden)
...
resolve_split_iden resolve_start name_map_stack (SIR.SplitIdentifier'Single i) = SIR.SplitIdentifier'Single <$> lift (lift $ resolve_start name_map_stack i)
