...
assign sir_child_maps (SIR.SIR mods adts type_synonyms classes instances type_vars variables mod) =
    ...
    pure (SIR.SIR mods adts synonyms classes instances type_vars (Arena.transform change_variable variables) mod)
    where
...
assign_in_expr nc_stack (SIR.Expr'Let id type_info sp bindings adts type_synonyms classes instances body) =
    ...
    NameMaps.make_name_maps_from_decls [] [] [] todo todo bindings adts type_synonyms classes >>= \ new_nm ->
