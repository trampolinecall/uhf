...
module UHF.Data.IR.TypeWithInferVar.PP.InferVarNamer (InferVarNamer, run_infer_var_namer, name_infer_var, make_infer_var_name_messages, tests) where
...
newtype InferVarNamer a = InferVarNamer (State (InferVarName, Map InferVarKey Text) a) deriving (Functor, Applicative, Monad)
...
name_infer_var var = InferVarNamer $
    state $
        \(cur_n, cache) ->
