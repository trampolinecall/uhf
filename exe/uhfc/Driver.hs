...
type AST = [AST.Decl]
type FirstIR = (Arena.Arena IR.Decl IR.DeclKey, Arena.Arena (IR.NominalType (IR.TypeExpr (Location.Located [Location.Located Text]))) IR.NominalTypeKey, Arena.Arena (IR.Binding (Location.Located [Location.Located Text])) IR.BindingKey, Arena.Arena IR.BoundName IR.BoundNameKey, IR.DeclKey)
