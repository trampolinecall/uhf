...
type GraphIR = (Arena.Arena IR.Decl IR.DeclKey, Arena.Arena (IR.NominalType (Maybe (IR.Type Void))) IR.NominalTypeKey, Arena.Arena IR.GraphNode IR.GraphNodeKey, Arena.Arena (IR.BoundValue (Maybe (IR.Type Void))) IR.BoundValueKey)
...
compile' file =
    ...
    let graph = (ToGraph.to_graph bound_values bindings)
    in pure (decls, nominal_types, graph, bound_values)
