        ...
        SIR.ID.run_t $
            runStateT
                ( do
                    ...
                    module_sir_id <- lift SIR.ID.gen_module_id
...
search_for_main_function mods variables mod =
    ...
    where
        ...
        go_pat (SIR.Pattern'Variable _ _ _ vk) = go_var vk
        go_pat (SIR.Pattern'Wildcard _ _ _) = []
        go_pat (SIR.Pattern'Tuple _ _ _ a b) = go_pat a ++ go_pat b
        go_pat (SIR.Pattern'Named _ _ _ _ (Located _ vk) subpat) = go_var vk ++ go_pat subpat
        ...
        go_pat (SIR.Pattern'Poison _ _ _) = []

        go_var vk =
            let (SIR.Variable _ _ _ (Located _ name)) = Arena.get variables vk
            in if name == "main" then [vk] else []

        get_var_span vk =
            let (SIR.Variable _ _ _ (Located sp _)) = Arena.get variables vk
            in sp
...
convert_decls var_parent decl_parent decls =
    ...
    where
        ...
        convert_decl ind (AST.Decl'Value _ target (Located eq_sp _) expr) =
            ...
            lift SIR.ID.gen_binding_id >>= \ binding_id ->
...
convert_type (AST.Type'Refer id) = lift SIR.ID.gen_type_expr_id >>= \ teid -> pure $ SIR.TypeExpr'Refer teid () (Const ()) (just_span id) () (convert_aiden_tok <$> id)
convert_type (AST.Type'Get sp prev name) = lift SIR.ID.gen_type_expr_id >>= \ teid -> convert_type prev >>= \ prev -> pure (SIR.TypeExpr'Get teid () (Const ()) sp prev (convert_aiden_tok <$> name))
convert_type (AST.Type'Tuple sp items) = mapM convert_type items >>= group_items
    where
        -- TODO: better spans for this
        group_items [a, b] = lift SIR.ID.gen_type_expr_id >>= \ teid -> pure $ SIR.TypeExpr'Tuple teid () sp a b
        group_items (a:b:more) = lift SIR.ID.gen_type_expr_id >>= \ teid -> SIR.TypeExpr'Tuple teid () sp a <$> group_items (b:more)
        group_items [_] = tell_error (Tuple1 sp) >> lift SIR.ID.gen_type_expr_id >>= \ teid -> pure (SIR.TypeExpr'Poison teid () sp)
        group_items [] = tell_error (Tuple0 sp) >> lift SIR.ID.gen_type_expr_id >>= \ teid -> pure (SIR.TypeExpr'Poison teid () sp)
convert_type (AST.Type'Hole sp id) = lift SIR.ID.gen_type_expr_id >>= \ teid -> pure $ SIR.TypeExpr'Hole teid () () sp (convert_aiden_tok <$> id)
convert_type (AST.Type'Function sp arg res) = SIR.TypeExpr'Function <$> lift SIR.ID.gen_type_expr_id <*> pure () <*> pure sp <*> convert_type arg <*> convert_type res
convert_type (AST.Type'Forall sp tys ty) =
    mapM (new_type_var . fmap convert_aiden_tok) tys >>= \case
        ...
        tyv1:tyv_more -> lift SIR.ID.gen_type_expr_id >>= \ teid -> SIR.TypeExpr'Forall teid () sp () (tyv1 :| tyv_more) <$> convert_type ty

convert_type (AST.Type'Apply sp ty args) =
    convert_type ty >>= \ ty ->
    foldlM (\ ty arg -> lift SIR.ID.gen_type_expr_id >>= \ teid -> SIR.TypeExpr'Apply teid () sp ty <$> convert_type arg) ty args -- TODO: fix spans
convert_type (AST.Type'Wild sp) = lift SIR.ID.gen_type_expr_id >>= \ teid -> pure $ SIR.TypeExpr'Wild teid () sp

convert_expr :: ID.ExprID -> AST.Expr -> MakeIRState Expr
convert_expr cur_id (AST.Expr'ReferAlpha sp t i) = lift SIR.ID.gen_expr_id >>= \ eid -> SIR.Expr'Refer eid cur_id () sp <$> make_split_identifier t (convert_aiden_tok <$> i)
convert_expr cur_id (AST.Expr'Char sp c) = lift SIR.ID.gen_expr_id >>= \ eid -> pure (SIR.Expr'Char eid cur_id () sp c)
convert_expr cur_id (AST.Expr'String sp s) = lift SIR.ID.gen_expr_id >>= \ eid -> pure (SIR.Expr'String eid cur_id () sp s)
convert_expr cur_id (AST.Expr'Int sp i) = lift SIR.ID.gen_expr_id >>= \ eid -> pure (SIR.Expr'Int eid cur_id () sp i)
convert_expr cur_id (AST.Expr'Float sp f) = lift SIR.ID.gen_expr_id >>= \ eid -> pure (SIR.Expr'Float eid cur_id () sp f)
convert_expr cur_id (AST.Expr'Bool sp b) = lift SIR.ID.gen_expr_id >>= \ eid -> pure (SIR.Expr'Bool eid cur_id () sp b)

convert_expr cur_id (AST.Expr'Tuple sp items) = group_items cur_id items
    where
        group_items cur_id [a, b] = lift SIR.ID.gen_expr_id >>= \ eid -> convert_expr (ID.ExprID'TupleFirstOf cur_id) a >>= \ a -> convert_expr (ID.ExprID'TupleSecondOf cur_id) b >>= \ b -> pure (SIR.Expr'Tuple eid cur_id () sp a b)
        group_items cur_id (a:b:more) = lift SIR.ID.gen_expr_id >>= \ eid -> convert_expr (ID.ExprID'TupleFirstOf cur_id) a >>= \ a -> SIR.Expr'Tuple eid cur_id () sp a <$> group_items (ID.ExprID'TupleSecondOf cur_id) (b:more) -- TODO: properly do span of b:more because this just takes the span of the whole thing
        group_items cur_id [_] = lift SIR.ID.gen_expr_id >>= \ eid -> tell_error (Tuple1 sp) >> pure (SIR.Expr'Poison eid cur_id () sp)
        group_items cur_id [] = lift SIR.ID.gen_expr_id >>= \ eid -> tell_error (Tuple0 sp) >> pure (SIR.Expr'Poison eid cur_id () sp)
            ...
            (\ ind (op, right) -> do
                ...
                case op of
                    AST.Operator'Path op_sp op_ty op_last -> do
                        ...
                        siid <- lift SIR.ID.gen_split_identifier_id
                        pure (op_sp, SIR.SplitIdentifier'Get siid op_ty (convert_siden_tok <$> op_last) (Const ()), right')
                    AST.Operator'Single op_iden@(Located op_sp _) -> do
                        siid <- lift SIR.ID.gen_split_identifier_id
                        pure (op_sp, SIR.SplitIdentifier'Single siid () (convert_siden_tok <$> op_iden) (Const ()), right'))
            [1..]
            ops
...
convert_expr cur_id (AST.Expr'If sp (Located if_sp _) cond t f) = lift SIR.ID.gen_expr_id >>= \ eid -> SIR.Expr'If eid cur_id () sp if_sp <$> convert_expr (ID.ExprID'IfCond cur_id) cond <*> convert_expr (ID.ExprID'IfTrue cur_id) t <*> convert_expr (ID.ExprID'IfFalse cur_id) f
...
convert_expr cur_id (AST.Expr'TypeAnnotation sp ty e) = lift SIR.ID.gen_expr_id >>= \ eid -> SIR.Expr'TypeAnnotation eid cur_id () sp <$> ((,()) <$> convert_type ty) <*> convert_expr (ID.ExprID'TypeAnnotationSubject cur_id) e
...
convert_expr cur_id (AST.Expr'TypeApply sp e args) =
    ...
    snd <$> foldlM
        ...
        args -- TODO: fix span for this
convert_expr cur_id (AST.Expr'Hole sp hid) = lift SIR.ID.gen_expr_id >>= \ eid -> pure (SIR.Expr'Hole eid cur_id () sp (convert_aiden_tok <$> hid))
...
convert_pattern parent (AST.Pattern'AlphaVar located_name@(Located name_sp (Token.AlphaIdentifier name))) =
    lift SIR.ID.gen_variable_id >>= \ vid ->
    lift SIR.ID.gen_pattern_id >>= \ pid ->
...
convert_pattern _ (AST.Pattern'Wildcard (Located underscore_sp _)) = lift SIR.ID.gen_pattern_id >>= \ pid -> pure (SIR.Pattern'Wildcard pid () underscore_sp)
convert_pattern parent (AST.Pattern'Tuple sp subpats) =
    ...
    where
        go [a, b] = lift SIR.ID.gen_pattern_id >>= \ pid -> pure $ SIR.Pattern'Tuple pid () sp a b
        go (a:b:more) = lift SIR.ID.gen_pattern_id >>= \ pid -> SIR.Pattern'Tuple pid () sp a <$> go (b:more)
        go [_] = lift SIR.ID.gen_pattern_id >>= \ pid -> tell_error (Tuple1 sp) >> pure (SIR.Pattern'Poison pid () sp)
        go [] = lift SIR.ID.gen_pattern_id >>= \ pid -> tell_error (Tuple0 sp) >> pure (SIR.Pattern'Poison pid () sp)
...
convert_pattern parent (AST.Pattern'AnonADTVariant sp v_ty variant fields) = do
    ...
    pid <- lift SIR.ID.gen_pattern_id
    pure (SIR.Pattern'AnonADTVariant pid () sp variant_split_iden [] fields)
convert_pattern parent (AST.Pattern'NamedADTVariant sp v_ty variant fields) = do
    ...
    variant_split_iden <- make_split_identifier v_ty (convert_aiden_tok <$> variant)
    pid <- lift SIR.ID.gen_pattern_id
    pure (SIR.Pattern'NamedADTVariant pid () sp variant_split_iden [] fields)
...
make_split_identifier Nothing i = do
    siid <- lift SIR.ID.gen_split_identifier_id
    pure $ SIR.SplitIdentifier'Single siid () i (Const ())
make_split_identifier (Just ty) i = do
    siid <- lift SIR.ID.gen_split_identifier_id
    ty <- convert_type ty
    pure $ SIR.SplitIdentifier'Get siid ty i (Const ())
