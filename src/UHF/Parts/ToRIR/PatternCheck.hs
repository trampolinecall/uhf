...
check adt_arena type_synonym_arena = mapAccumL check_one_pattern [Any]
    where
        ...
        check_against_one_uncovered_value (SIR.Pattern'AnonADTVariant (Just ty) _ _ _ (Just pat_variant) _ pat_fields) uncovered_value =
            case uncovered_value of
                Any ->
        ...
        check_against_one_uncovered_value (SIR.Pattern'AnonADTVariant _ _ _ _ _ _ _) uncovered_value = ([uncovered_value], []) -- a variant pattern with an unknown variant or an unknown type does not cover anything

        check_against_one_uncovered_value (SIR.Pattern'NamedADTVariant (Just ty) _ _ _ (Just variant) _ fields) uncovered_value = todo
        check_against_one_uncovered_value (SIR.Pattern'NamedADTVariant _ _ _ _ _ _ _) uncovered_value = ([uncovered_value], []) -- same as above: a variant pattern with an unknown variant or an unknown type does not cover anything
