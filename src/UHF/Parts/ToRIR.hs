...
convert_expr (SIR.Expr'Refer id ty sp _ _ bv) = do
...
convert_expr (SIR.Expr'Match id ty sp match_tok_sp scrutinee arms) = do
    ...
    where
        ...
        pattern_to_clauses scrutinee_var (SIR.Pattern'AnonADTVariant ty sp _ _ variant_index tyargs fields) =
        ...
        pattern_to_clauses _ (SIR.Pattern'NamedADTVariant _ _ _ _ _ _ _) = todo
...
assign_pattern incomplete_err_sp pat expr = do
    ...
    where
        ...
        go (SIR.Pattern'AnonADTVariant ty sp _ _ variant tyargs fields) expr = todo
        go (SIR.Pattern'NamedADTVariant ty sp _ _ variant tyargs fields) expr = todo
