...
data Expr
    = Expr'Identifier Type Span BoundValueKey
    | Expr'Char Type Span Char
    | Expr'String Type Span Text
    | Expr'Int Type Span Integer
    | Expr'Float Type Span Rational
    | Expr'Bool Type Span Bool -- TODO: replace with identifier exprs

    | Expr'Tuple Type Span Expr Expr

    | Expr'Lambda Type Span Pattern Expr

    | Expr'Let Type Span [Binding] Expr

    | Expr'Call Type Span Expr Expr

    | Expr'If Type Span Span Expr Expr Expr -- TODO: desugar to 'case x { true -> _; false -> _ }
    | Expr'Case Type Span Span Expr [(Pattern, Expr)] -- TODO: case should be more like switch at this level

    | Expr'Poison Type Span
    deriving Show

data Pattern
    = Pattern'Identifier Type Span BoundValueKey
    | Pattern'Wildcard Type Span
    | Pattern'Tuple Type Span Pattern Pattern
    | Pattern'Named Type Span Span (Located BoundValueKey) Pattern

    | Pattern'Poison Type Span
    deriving Show
...
expr_type (Expr'If ty _ _ _ _ _) = ty
expr_type (Expr'Case ty _ _ _ _) = ty
...
pattern_type (Pattern'Named ty _ _ _ _) = ty
