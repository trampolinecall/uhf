...
data Variant ty
    ...
    deriving Show
data VariantIndex = VariantIndex {- TODO: should this be added back in? DoNotConstruct -} ADTKey Int deriving (Show, Eq, Ord)
data FieldIndex = FieldIndex {- TODO: should this be added back in? DoNotConstruct -} VariantIndex Int deriving (Show, Eq, Ord)
...
variant_field_idxs arena v_idx =
    ...
    in case variant of
        (Variant'Anon _ _ fields) -> fields & zipWith (\ i _ -> FieldIndex v_idx i) [0..]
        (Variant'Named _ _ fields) -> fields & zipWith (\ i _ -> FieldIndex v_idx i) [0..]
...
-- technically is partial, but hopefully because VariantIndexes are only constructed from SIR.ADTVariantIndex, the variant can be guaranteed to exist
